package com.playjava.StoreWarehouse.bd;
...
public class SearchFor extends OutputTable {
	...
	public void searchNemeByGroup(String group) throws ClassNotFoundException,
			SQLException {
		sum = 0;
		try {
			connection();// create a database connection
			// ===========Search for name_group of goods
			rs = statement
					.executeQuery("SELECT * FROM Goods WHERE name_group = '"
							+ group + "'");
			out(rs); // вивод даних
		} catch (SQLException e) {
			// if the error message is "out of memory",
	...
	public void tePrintSearchGroup() throws ClassNotFoundException,
			SQLException {
		try {
			connection();// create a database connection
			// ===========Search for name_group of goods
			rs = statement
					.executeQuery("SELECT DISTINCT name_group FROM Goods");
	...
	public ArrayList searchSumByGroup() throws ClassNotFoundException,
			SQLException {
		...
		try {
			...
			// ===========Search for name_group of goods
			rs = statement
					.executeQuery("SELECT name_group, sum(count) as count, "
